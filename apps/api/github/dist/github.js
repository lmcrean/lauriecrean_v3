    ...
    try {
        ...
        for (let githubPage = 1; githubPage <= githubPagesNeeded; githubPage++) {
            ...
            if (itemsToFetchThisPage <= 0)
                break;
            try {
                const { data: searchResults } = await retryApiCall(() => octokit.rest.search.issuesAndPullRequests({
                    q: searchQuery,
                    sort: 'created',
                    order: 'desc',
        ...
        if (allSearchItems.length === 0) {
            ...
            const { data: countSearch } = await retryApiCall(() => octokit.rest.search.issuesAndPullRequests({
                q: searchQuery,
                sort: 'created',
                order: 'desc',
                per_page: 1,
        ...
        for (let i = 0; i < maxDetailedCalls; i++) {
            ...
            try {
                ...
                const { data: pr } = await retryApiCall(() => octokit.rest.pulls.get({
                    owner,
                    repo,
                    pull_number: item.number
                }));
                // Get repository details with retry logic
                const { data: repoData } = await retryApiCall(() => octokit.rest.repos.get({
                    owner,
                    repo
                }));
                // Add small delay between API calls to be respectful
...
async function getPullRequestDetails(owner, repo, pullNumber) {
    try {
        ...
        const [prResponse, commentsResponse] = await Promise.all([
            retryApiCall(() => octokit.rest.pulls.get({
                owner,
                repo,
                pull_number: pullNumber
            })),
            retryApiCall(() => octokit.rest.issues.listComments({
                owner,
                repo,
